...
**规范说明**：mall-agent 不再包含 `api/` 目录，所有对外接口定义在 `mall-agent-api` 模块中。
...
| **远程 Request** | - | - | `api/dto/request/` |
| **远程 Response** | 引用 API 模块 | - | `api/dto/response/` |
| **FeignClient** | 引用 API 模块 | - | `api/feign/` |
...
├── feign/
│   └── AgentFeignClient.java          # Feign 客户端
└── api/
    └── dto/
        ├── request/
        │   └── AgentListRequest.java    # 查询请求（多RequestParam或RequestBody）
        ├── response/
        │   └── AgentResponse.java       # 简单响应（复用应用服务Response）
        └── vo/
...
│   └── inner/
│       └── AgentInnerController.java  # 内部 Feign 接口（参数转换后直接转发Service）
├── service/
...
│       └── response/
│           └── AgentApiResponse.java       # Controller出参（Service层转换后返回）
└── config/
    └── FeignConfig.java
```
